...
/**
 * JSON sibling of App\Http\Controllers\EventInvitationDesignController — no session, no
 * redirect+flash. update()'s transaction body is copied verbatim from the web controller
 * (see the Slice C2 plan); only the top-of-method ticketed/needs-template guard and the
 * final response shape differ. The web controller is untouched by this class.
 */
class EventInvitationDesignController extends Controller
{
    public function show(Request $request, Event $event, InvitationCustomizationService $customizationService): JsonResponse
    {
        ...
        // merge() -> resolvedTemplate() already called $event->setRelation('invitationTemplate', ...)
        // as a side effect, so this is already loaded — no extra query.
        $template = $event->getRelation('invitationTemplate');
    ...
    public function update(
        ...
        // UpdateInvitationDesignRequest is deliberately NOT type-hinted on this method:
        // Laravel resolves (and auto-validates) a type-hinted FormRequest before the
        // method body ever runs, which would let its own withValidator() silently fall
        // back to "the first active template" for a no-template event before this guard
        // gets a chance to run. Resolving it manually, after the guard, defers that
        // validation to here on purpose. Since that also defers
        // UpdateInvitationDesignRequest::authorize()'s policy check, it must be repeated
        // explicitly here first — otherwise an unauthorized caller could learn an
        // event's ticketed/template state before ever being gated.
        $this->authorize('update', $event);
        ...
        /** @var UpdateInvitationDesignRequest $designRequest */
        $designRequest = app(UpdateInvitationDesignRequest::class);
        $request = $designRequest;
        $validated = $designRequest->validated();
        ...
        $fresh = $event->fresh();
        $invitation = $customizationService->merge($fresh);
        $template = $fresh->getRelation('invitationTemplate');
     ...
     * @return array<string, mixed>
     */
    private function resolveMediaUrls(array $invitation): array
    {
