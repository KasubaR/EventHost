...
function mountCharts(root, payload) {
    ...
    if (Array.isArray(dailySeries)) {
        const wrap = root.querySelector('[data-admin-chart="daily"]');
    ...
    const monthly = payload.monthly_registrations;
    if (Array.isArray(monthly) && monthly.length > 0) {
        const wrap = root.querySelector('[data-admin-chart="monthly_users"]');
        const canvas = wrap?.querySelector('canvas');
        if (canvas instanceof HTMLCanvasElement) {
            teardownCanvas(canvas);
            instances.push(
                new Chart(canvas, {
                    type: 'bar',
                    data: {
                        labels: monthly.map((r) => String(r.label ?? '')),
                        datasets: [
                            {
                                label: 'New users',
    ...
    const weekly = payload.weekly_events_created;
    if (Array.isArray(weekly) && weekly.length > 0) {
        const wrap = root.querySelector('[data-admin-chart="weekly_events"]');
        const canvas = wrap?.querySelector('canvas');
        if (canvas instanceof HTMLCanvasElement) {
            teardownCanvas(canvas);
            instances.push(
                new Chart(canvas, {
                    type: 'bar',
                    data: {
                        labels: weekly.map((r) => String(r.label ?? '')),
                        datasets: [
                            {
                                label: 'Events created',
                                data: weekly.map((r) => Number(r.count ?? 0)),
    ...
    if (Array.isArray(statusRows) && statusRows.length > 0) {
        const wrap = root.querySelector('[data-admin-chart="status"]');
        const canvas = wrap?.querySelector('canvas');
        if (canvas instanceof HTMLCanvasElement) {
            teardownCanvas(canvas);
            const colorsByKey = {
    ...
    if (Array.isArray(eventTypes) && eventTypes.length > 0) {
        const wrap = root.querySelector('[data-admin-chart="event_types"]');
        const canvas = wrap?.querySelector('canvas');
        if (canvas instanceof HTMLCanvasElement) {
            teardownCanvas(canvas);
            instances.push(
                new Chart(canvas, {
                    type: 'doughnut',
                    data: {
                        labels: eventTypes.map((r) => String(r.label ?? '')),
                        datasets: [
                            {
...
function boot() {
    document.querySelectorAll('[data-admin-analytics-root]').forEach((root) => {
        if (!(root instanceof HTMLElement)) {
            return;
        }
        const jsonId = root.dataset.adminAnalyticsJsonId;
        if (!jsonId || root.dataset.adminAnalyticsChartsReady === '1') {
            return;
        }
        ...
        try {
            ...
            root.dataset.adminAnalyticsChartsReady = '1';
        } catch (err) {
            console.error('[admin-analytics-charts] Failed to mount charts', err);
        }
    });
}
